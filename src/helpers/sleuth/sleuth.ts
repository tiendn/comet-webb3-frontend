...
const sleuthDeployer = '0x84C3e20985d9E7aEc46F80d2EB52b731D8CC40F8';
...
export class Sleuth {
  ...
  constructor(provider: Provider, opts: Opts = {}) {
    ...
    //TODO: We shoud update sleuth so clients don't have to worry about where this address should come from.
  ...
  static querySolOutput<T, A extends any[] = []>(c: SolcContract, opts: SolidityQueryOpts = {}): Query<T, A> {
    const queryFunctionName = opts.queryFunctionName ?? 'query';
    const b = c.evm?.bytecode?.object ?? c.bytecode?.object;
    if (!b) {
      throw new Error(`Missing (evm.)bytecode.object in contract ${JSON.stringify(c, null, 4)}`);
    }
    const abi = c.abi;
    const queryAbi = abi.find(({ type, name }: any) => type === 'function' && name === queryFunctionName);
    if (!queryAbi) {
  ...
  static querySolSource<T, A extends any[] = []>(q: string, opts: SolidityQueryOpts = {}): Query<T, A> {
    const fnName = opts.queryFunctionName ?? 'query';
    const input = {
      language: 'Solidity',
      sources: {
        'query.sol': {
  ...
  async fetch<T, A extends any[] = []>(q: Query<T, A>, args?: A): Promise<T> {
    const sleuthCtx = new Contract(
      this.sleuthAddr,
      ['function query(bytes,bytes) public view returns (bytes)'],
      this.provider
    );
    const iface = new Interface([q.fn]);
    const argsCoded = iface.encodeFunctionData(q.fn.name, args ?? []);
    const queryResult = await sleuthCtx.query(hexify(q.bytecode), argsCoded);
    const r = this.coder.decode(q.fn.outputs ?? [], queryResult) as unknown;
